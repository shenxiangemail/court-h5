<template>
	<div>
		<div>
			<div ref="adbox" class="left_fixed" style="display: flex;flex-direction: column;justify-content: flex-start;" v-show="active1 && imgSrc1 !=''"
			 v-if="imgSrc1.isFloating !=1">
				<div style="display: flex;" class="left_image_active"><img :src="imgSrc1.pageUrl" :style="{'width':imgSrc1.width +'px','height':imgSrc1.height +'px'}"
					 @click="openUrl(imgSrc1.linkUrl)"></div>
				<div style="text-align: left;font-size: 1.125rem;padding-top: 0.5rem" class="active_color" v-on:click="close(1)">关闭</div>
			</div>
			<div v-show="active1 && imgSrc1 !=''" v-if="imgSrc1.isFloating ==1">
				<testLeft time='0' x1='10' y1='200' :linkUrl='imgSrc1.linkUrl' :width='imgSrc1.width' :height='imgSrc1.height' :url='imgSrc1.pageUrl'
				 style="width: 100%;"></testLeft>
			</div>

			<div ref="adbox1" class="left_fixed" style="display: flex;flex-direction: column;justify-content: flex-start;margin-top: 30px;"
			 v-show="active2 && imgSrc2 !=''" v-if="imgSrc2.isFloating != 1">
				<div style="display: flex;" class="left_image_active"><img :src="imgSrc2.pageUrl" :style="{'width':imgSrc2.width +'px','height':imgSrc2.height +'px'}"
					 @click="openUrl(imgSrc2.linkUrl)"></div>
				<div style="text-align: left;font-size: 1.125rem;padding-top: 0.5rem" class="active_color" v-on:click="close(2)">关闭</div>
			</div>;
			<div v-show="active2 && imgSrc2 !=''" v-if="imgSrc2.isFloating ==1">
				<testLeft time='5000' x1='10' y1='450' :linkUrl='imgSrc2.linkUrl' :url='imgSrc2.pageUrl' :width='imgSrc2.width'
				 :height='imgSrc2.height'></testLeft>
			</div>

			<div ref="adbox2" class="left_fixed" style="display: flex;flex-direction: column;justify-content: flex-start;margin-top: 10px;"
			 v-show="active3 && imgSrc3 !=''" v-if="imgSrc3.isFloating != 1 ">
				<div style="text-align: left;font-size: 1.125rem;padding-top: 0.5rem" class="active_color" v-on:click="close(3)">关闭</div>
				<div style="display: flex;" class="left_image_active"><img :src="imgSrc3.pageUrl" :style="{'width':imgSrc3.width +'px','height':imgSrc3.height + 'px'}"
					 @click="openUrl(imgSrc3.linkUrl)"></div>
			</div>
			<div v-show="active3 && imgSrc3 !=''" v-if="imgSrc3.isFloating ==1">
				<testLeft time='10000' x1='20' y1='650' :linkUrl='imgSrc3.linkUrl' :url='imgSrc3.pageUrl' :width='imgSrc3.width'
				 :height='imgSrc3.height'></testLeft>
			</div>
		</div>
	</div>
</template>
...
<script>
	import testLeft from './test_left.vue'
	export default {
