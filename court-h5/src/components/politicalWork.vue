<template>
	<div class="politicalWork_body">
		<div class="politicalWork_body_top" :style="courtNewsBodyStyle">
			<div class="politicalWork_body_title">
				<div class="politicalWork_body_title_t politicalWork_title_name_active" @mouseover="titleMouseover(title[index].own)"
				 @click="toNewsList(title[index].own)">{{title[index].name}}</div>
				<div class="politicalWork_body_title_two">
					<div class="politicalWork_title_name_active" style="color: #53b6f8" @mouseover="titleMouseover(twoTitleFirst)"
					 @click="toNewsList(twoTitleFirst)">{{twoTitleFirst.menuName}}&nbsp;</div>
					<div class="politicalWork_title_name_active" style="color: #53b6f8" v-show="Object.keys(twoTitleSecond).length > 0"
					 @mouseover="titleMouseover(twoTitleSecond)" @click="toNewsList(twoTitleSecond)">|&nbsp;{{twoTitleSecond.menuName}}&nbsp;</div>
		...
		<div class="politicalWork_content">
			<ul style="color: #53b6f8;margin-block-start: 0rem;margin-block-end: 0;padding-top: 10px;padding-inline-start: 20px;padding-left: 20px;">
				...
				<template v-for="item in informationTitles">
					<li style="padding-top: 8px;list-style: none;" @click="toNewsDeatail(item)">
						<div style="display: flex;;color: #333333;overflow: hidden;" class="politicalWork_title_active">
							<div style="display: flex;align-items: center;">
								<div style="font-size: 10px;border-radius: 50%;width: 4px;height: 4px;background-color:  #53b6f8;margin-right: 15px;"></div>
								<div :class="index > 3 ? 'title-name-length' : 'title-name-length2'">{{item.titleName}}</div>
							</div>
		...
		<div style="color: #b0b0b0;display:flex;justify-content: flex-end; padding-right: 20px;padding-top: 10px; padding-bottom: 10px;border-bottom:  #dddddd solid 0.0625rem;
 border-left: #dddddd solid 0.0625rem;border-right: #dddddd solid 0.0625rem;">责任部门：{{responsibleDepartment}}</div>
	</div>
</template>
...
<style scoped>
	...
	.politicalWork_body_top {
		background-size: cover;
	}

	.politicalWork_body_title {
		display: flex;
		/* align-items: center; */
	...
	.politicalWork_title_active:hover {
		color: #53b6f8 !important;
		cursor: pointer;
	}

	.politicalWork_body_title_active:hover {
		color: #920784 !important;
		cursor: pointer;
	}

	.politicalWork_title_name_active:hover {
		cursor: pointer;
